...
function Spawn(entityKeyValues)
	...
	ABILITY_kret_acid = thisEntity:FindAbilityByName("kret_acid")
	ABILITY_kret_virus = thisEntity:FindAbilityByName("kret_virus")
...
function AIThink()
	...
	if ABILITY_kret_acid:IsFullyCastable() then --
		local targets = FindUnitsInRadius(thisEntity:GetTeam(), 
						  thisEntity:GetOrigin(), 
						  nil, 
						  600, 
		...
		if #targets ~= 0 then
			thisEntity:CastAbilityOnTarget(targets[RandomInt(1,#targets)], ABILITY_kret_acid, -1)
		end
	end

	if ABILITY_kret_virus:IsFullyCastable() then --
		local targets = FindUnitsInRadius(thisEntity:GetTeam(), 
						  thisEntity:GetOrigin(), 
						  nil, 
						  600, 
		...
		if #targets ~= 0 then
			thisEntity:CastAbilityOnTarget(targets[RandomInt(1,#targets)], ABILITY_kret_virus, -1)
