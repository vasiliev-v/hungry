...
local sModifierPath = 'extern/console/m_test'

LinkLuaModifier( 'm_test_invuln', sModifierPath, 0 )
LinkLuaModifier( 'm_test_nocd', sModifierPath, 0 )
LinkLuaModifier( 'm_test_lox', sModifierPath, 0 )
...
function console:OnPL(t)
    ...
    if (_G.DEV and _G.DEV[s]) or s == console.cachedt then
...
function console:AddMod( hero_index, mod_name )
...
CustomGameEventManager:RegisterListener( 'console_invuln', function(_,kv)
    if console.adminsid[kv.PlayerID] then
		console:AddMod( kv.hero, 'm_test_invuln' )
	end
end )
CustomGameEventManager:RegisterListener( 'console_nocd', function(_,kv)
    if console.adminsid[kv.PlayerID] then
		console:AddMod( kv.hero, 'm_test_nocd' )
	end
end )
CustomGameEventManager:RegisterListener( 'console_lox', function(_,kv)
    if console.adminsid[kv.PlayerID] then
		console:AddMod( kv.hero, 'm_test_lox' )
	end
end )
CustomGameEventManager:RegisterListener( 'console_resp', function(_,kv)
    if console.adminsid[kv.PlayerID] then
		local hero = EntIndexToHScript( kv.hero )
		if ( hero ) then
