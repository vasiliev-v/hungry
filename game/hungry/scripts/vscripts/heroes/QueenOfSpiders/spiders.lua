	...
	if ability and ability:GetAutoCastState() then
		...
		if randUnit ~= nil and not randUnit:HasModifier(modifier) then
			if ability:IsCooldownReady() and not caster:IsChanneling() then
				caster:CastAbilityOnTarget(randUnit, ability, -1)
			end
...
function spiderCreate(keys)
	...
	--
	for i=1,count do
		local cre = CreateUnitByName( "queen_of_spiders_spider_3", target:GetAbsOrigin(), false, caster, caster, caster:GetTeam())
