...
function AICreepsAttackOneUnit(params)
	...
	if not AICreepsCheck_tHeroTarget_isNotHave({index = index}) then
		...
		if (unit:GetOrigin() - unit.vSpawn):Length2D() > 600 then
			AttackMove(unit, unit.vSpawn)
		...
		if (unit:GetOrigin() - unit.vSpawn):Length2D() > 600 then
			AttackMove(unit, unit.vSpawn, DOTA_UNIT_ORDER_MOVE_TO_POSITION)
			return 2
		end
	end
